...
const { action, submit: _submit = async () => { },
  ...
  submit?: (ctx: context123123) => Promise<T>,
...
<template>
  ...
  <el-table m="t-15px" :data="data && data[listKey] || []" v-loading="loading || submiting"
  ...
  <el-form ref="ruleFormRef" :model="form" :="{ ..._form, ...formProps }">
    ...
    <el-drawer v-model="drawer.show" @open="ruleFormRef?.clearValidate()" @close="close" :="drawerProps">
      ...
      <template #footer>
        <slot :name="`${drawer.slot}-footer`" :="ctx" :loading="loading" :submiting="submiting">
          ...
          <el-button type="primary" @click="submit" :disabled="submiting">确定</el-button>
        </slot>
      </template>
    </el-drawer>

    <el-dialog v-model="dialog.show" @open="ruleFormRef?.clearValidate()" @close="close" :="dialogProps">
      <div v-loading="submiting">
